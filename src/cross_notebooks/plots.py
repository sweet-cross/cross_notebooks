...
class Plots:
        ...
        for v in subcats:
            ...
            for resolution in v["time_resolution"]:
                ...
                for item in v["data"]:
                    ...
                    existing_cat = d.loc[
                        d["use_technology_fuel"] == cat, base_keys
                    ].drop_duplicates()
                    ...
                    if resolution == "annual":
                        ...
                        if not existing_cat.empty:
                            existing_cat["timestamp"] = pd.to_datetime(
                                existing_cat["timestamp"],
                                dayfirst=True,
                                errors="coerce",
                            ).dt.floor("min")

                    if not existing_cat.empty:
                        sub_sum = sub_sum.merge(
                            existing_cat, on=base_keys, how="left", indicator=True
                        )
                        sub_sum = sub_sum.loc[sub_sum["_merge"] == "left_only"].drop(
                            columns="_merge"
                        )
    ...
    def __calculateTotalSupply(self, varList_supply):
        ...
        total = d.groupby(keys, as_index=False)["value"].sum(
            min_count=1
